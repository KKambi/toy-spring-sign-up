...
public class SampleDao implements ISampleDao {
	private HashMap<String, Course> sampleMap;
	
	public SampleDao() {
		sampleMap = new HashMap<String, Course>();
	}
	
	public void sampleInsert(ClassPathResource file) {
		...
		for (int i=0; i<txtLine.length; i++) {
			...
			sampleMap.put(course.getCourseCode(), course);
		}
	};
	
	public HashMap<String, Course> sampleSelect(){
		//샘플맵을 리턴하는 함수
		return sampleMap;
	};
